...
- [x] 1枚に合成するオーバーレイをGrad-CAMに変更（ユーザー質問「一枚の活性化画像は判断結果の重みを乗算して合成したものか」への回答を経て2026-08-05対応）。予測クラスのスコアを対象層の活性化で微分し、チャンネルごとの重要度（寄与度）で重み付き合成することで「予測にどれだけ効いたか」を可視化。単純平均より分類根拠に忠実
  ...
  - `getGradCamForAllLayers`：`tf.grad`で予測スコアを活性化で微分し、チャンネル重み`channelWeights`を算出。重み付き合成→ReLUで正の寄与のみ残し0〜1正規化（src/mobilenet.ts）
...
- [x] パフォーマンス改善：層切り替えのたびにGrad-CAMを再計算しており重かった（ユーザー指摘により2026-08-05対応）。原因は(1)活性化モデルが毎回ほぼ全層分を計算、(2)予測クラスの特定に後半モデルの推論を毎回2回走らせていたこと。「画像は同じなら結果も同じ」というユーザーの指摘を踏まえ、`getGradCamForAllLayers`で画像読み込み時に14層すべてのGrad-CAMを一括計算（活性化の計算と予測クラスの特定は1回だけ、各層の勾配計算のみ繰り返す）し、層切り替えは計算済み結果を描画するだけに変更（src/mobilenet.ts, src/App.tsx）。計算中は「各層を解析中…」を表示。ブラウザ実行で層切り替えが約290ms/回→約19ms/回に高速化したことを確認
